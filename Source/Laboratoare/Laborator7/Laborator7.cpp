...
void Laborator7::Update(float deltaTimeSeconds)
{
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix, glm::vec3(1, 1, 0));
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint locsh = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(locsh, materialShininess);
	...
	GLint lococ = glGetUniformLocation(shader->program, "object_color");
	glUniform3fv(lococ, 1, glm::value_ptr(color));
