	...
	if (window->KeyHold(GLFW_KEY_1))
		{
			...
			if (!isortho)
			{
				projectionMatrix = glm::perspective(RADIANS(fov), 2.f, 1.f, 100.f);
			}
		}
		...
		if (window->KeyHold(GLFW_KEY_3))
		{
			...
			if (isortho)
			{
				projectionMatrix = glm::ortho(-10.f, 10.f, x, y, 0.f, 100.f);
			}
		}
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_O)
	{
		projectionMatrix = glm::ortho(-10.f, 10.f, -5.f, 5.f, 0.f, 100.f);
		isortho = true;
	}
	if (key == GLFW_KEY_P)
	{
		projectionMatrix = glm::perspective(RADIANS(40.f), 2.f, 1.f, 100.f);
		isortho = false;
	}
}
