...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		mix = true;
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(2.0f, 2, 0.0f));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"], mapTextures["bamboo"]);
		mix = false;
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	int mixLocation = glGetUniformLocation(shader->program, "mix_texture");
	glUniform1i(mixLocation, mix);
	...
	if (texture1)
	{
		...
		int text_location = glGetUniformLocation(shader->program, "texture_1");
	...
	if (texture2)
	{
		...
		//TODO : Send texture uniform value
		int text_location = glGetUniformLocation(shader->program, "texture_2");
		glUniform1i(text_location, 1);
	}
